// Build the share-route Lambda zip.
//
// Run from `apps/web/`:   node lambda/share-route/build.mjs
// Or from CI:             same.
//
// Output: apps/web/lambda/share-route/dist/share-route.zip
//
...
// fast with the install command — see apps/web/lambda/share-route/README.md.
...
if (!existsSync(spaShellPath)) {
	console.error(
		`[share-route build] missing SPA shell at ${spaShellPath}. ` +
...
execSync('zip -qr share-route.zip index.mjs index.mjs.map package.json node_modules', {
	cwd: distDir,
	stdio: 'inherit',
});

console.log(`built: ${resolve(distDir, 'share-route.zip')}`);
...
function copyResvgPackages() {
	...
	try {
		...
		console.error(
			`[share-route build] missing ${RESVG_ARM64}. The Lambda runs on ` +
