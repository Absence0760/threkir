...
4b. **Create the Services ID — the web half of Sign in with Apple.** Identifiers → **Services IDs** → `com.threkir.web`, with the App ID above as its primary. Configure it with the domain `threkir.com` and the return URL `https://<project-ref>.supabase.co/auth/v1/callback` — **Supabase's** callback, not ours, and Apple refuses `http://`, so there is no localhost entry to add. Then Keys → a **second** key with Sign in with Apple enabled, bound to the same primary App ID; its `.p8` is downloadable once and is **not** the APNs key. Those four values (Services ID, Team ID, Key ID, `.p8`) go into Supabase → Authentication → Providers → Apple. This is what unblocks the web Apple button and `apps/mobile_ios`'s `_kAppleSignInEnabled`; the ordered version with what to verify afterwards is [`docs/testing/e2e_dev_accounts.md § 2`](../../docs/testing/e2e_dev_accounts.md).
