...
2. **Services ID** — Identifiers → **Services IDs** → `com.threkir.web`. A *different identifier type* from the App ID, which is why it cannot be ticked on one. Set the App ID above as its primary, then Configure:
   ...
   This same Services ID is what **Android** needs as the `APPLE_SERVICE_CLIENT_ID` dart-define (with `APPLE_REDIRECT_URI`) — `appleSignInAvailable()` in `apps/mobile_android/lib/apple_auth.dart` gates the button on those two the way Google's is gated on `GOOGLE_WEB_CLIENT_ID`. iOS needs neither: it takes the native flow off the App ID capability.
