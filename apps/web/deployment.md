...
|---|---|---|
| `ANTHROPIC_API_KEY` | sops-encrypted in the private estate repo (env-specific AWS KMS key), then re-encrypted by Terraform into `aws_kms_ciphertext.coach` | server-only — never a plaintext Lambda env var; `/api/coach` decrypts the blob at cold start ([decisions § 1659](../../docs/architecture/decisions.md)) |
