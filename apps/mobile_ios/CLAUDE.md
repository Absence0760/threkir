...
> **Heading for the App Store** (deferral lifted 2026-09-21, [decisions § 1701](../../docs/architecture/decisions.md)). `release-ios.yml` signs this app and the Apple Watch app it embeds and uploads them to TestFlight; the operator steps are [`docs/ops/apple_provisioning.md`](../../docs/ops/apple_provisioning.md) 2–4 and 14–18, and the listing checklist is [`deployment.md`](deployment.md#production-readiness-checklist). The app is **iPhone-only** for now. Two App Review rules shape shared Dart here, and both are gated in code rather than by convention ([decisions § 1700](../../docs/architecture/decisions.md)): no iOS surface may link to a web page that takes payment for Pro or a donation to Threkir (`store_links.dart`'s `webPaymentLinksAllowed()`), and iOS renders no Google sign-in button unless the flow can work (`google_auth.dart`'s `googleSignInOffered()`). A new payment or sign-in surface on mobile goes through those two gates. Twin-parity edits made on Android are still mirrored here in the same commit.
