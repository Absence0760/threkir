...
tests-e2e/
  ...
  dashboard/tap-targets.spec.ts — every icon button on /dashboard measured against the 44 px bar at 1280x720 and 375x667, through fixtures/tap-targets.ts (which comment-tap-targets.spec.ts reads too). Fails a control that misses its floor OR meets it exactly — the headroom half, see decisions § 1656 — and fails a sub-bar control that is not in the inline-disclosure registry
  feed.spec.ts                 — /feed
