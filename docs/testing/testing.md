...
2. *The hook has nothing to call.* GoTrue then invokes `[auth.hook.send_email]`
   at `http://host.docker.internal:24321/functions/v1/auth-email`. There may be
...
- No mocks for databases we control — local Supabase (24322) is the authoritative fixture. Mock only third-party HTTP (Strava, Stripe).
