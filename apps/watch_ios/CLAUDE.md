...
> **Actively worked, and device-gated rather than deferred.** The public landing page still lists Apple Watch as "Coming soon", but the tier is being brought up to Wear OS parity under [issue #950](https://github.com/Absence0760/threkir/issues/950) — #951, #953, #954, #955 and #956 landed in 2026-09, and sign-in, the starred-route picker, spoken cues, live race mode and the complication's Widget Extension landed 2026-09-21. What genuinely blocks the remaining work is **hardware**: a paired Apple Watch and iPhone, tracked in #922. A simulator can build and test everything here; it cannot wear it. See [../../docs/product/parity.md](../../docs/product/parity.md) for the per-feature state and [decisions.md § 1](../../docs/architecture/decisions.md) for why this is a native target.
...
- `WatchConnectivityManager.swift` — Watch Connectivity framework, two-way messaging with the iOS phone app. `session(_:didReceiveMessage:)` and `session(_:didReceiveUserInfo:)` both funnel into one `receive(_:)` that applies each known key independently (`preferred_unit`, the armed-route push, the `saved_routes` picker list, the `audio_cues` toggle, the three settings `PhoneSettings.swift` stores, and live race mode's Arm / Go / End — see `ArmedRoute.swift` and `LiveRace.swift`); it also exposes `armRoute(_:)` for a wrist-side pick
- `PhoneSettings.swift` — the three settings the phone pushes beyond the unit and the cue switch, each stored on arrival because the recorder reads them long after the push lands (#950). `DefaultActivityType` — the pre-run picker opens on it, and `primed` moves an idle picker the runner has not touched, never mid-run and never over a wrist pick (Wear's two exceptions). `PrivacyDefault` — `stop()` and every 15 s checkpoint snapshot it as `runs.is_public` (`public → true`, else false, never pushed → nil and the key is omitted). `HeartRateZones` — the live `Z1`…`Z5` badge under the BPM, against a ladder the PHONE resolves from `hr_zones` > `max_hr_bpm` > Tanaka off `date_of_birth`; the date of birth never reaches this device. No ladder is no badge, as on Wear OS (decisions § 1245). None has an editor here
...
- `ArmedRoute.swift` — the phone → watch route push, and the picker's list: `SavedRoutes` (the `saved_routes` array of those same five-key dictionaries, bounded at 12 routes × 128 positions) plus the `SavedRoutesStore` UserDefaults slot that keeps the list across launches so a watch out of phone range still has one to offer. The single push: the `ArmedRoute` model (`route_id` / `route_name` / `route_distance_m` / two parallel `[Double]` coordinate arrays), its fail-closed `decode(_:)` of a `WCSession` payload, and the `ArmedRouteStore` UserDefaults slot it survives in between the push and the run that follows it
...
- `WatchAuth.swift` — the wrist's own Supabase session: GoTrue's password + refresh grants, `WatchAuthFault` (a port of Wear's `AuthFault`), the fail-closed `SupabaseEnvironment`, and the Keychain-backed `WatchSessionStore`. Two grants and nothing else — this is not where the Supabase surface grows
- `SignInView.swift` — the sign-in sheet: `TextField` + `SecureField` over watchOS's own text entry (dictation / Scribble / keyboard — there is no in-app IME to embed the way Wear OS does), plus the signed-in state and Sign out. Presented from the pre-run account row
...
- **Complication**: the `WatchAppComplication` Widget Extension target ships in both
  `WatchApp.xcodeproj` and `Runner.xcodeproj` as of 2026-09-21, so the complication source
  ...
  together ([decisions § 1717](../../docs/architecture/decisions.md)). The extension's own
  ...
  there showed kilometres to a runner who chose miles ([decisions § 1713](../../docs/architecture/decisions.md)).
...
- [ ] watchOS complication: pace + distance — Swift source (provider + four `widgetFamily` views) ships in `apps/watch_ios/Complications/`. `WorkoutManager.publishComplicationSnapshot` writes the active-run state to an App-Group `UserDefaults` on every transition (start / pause / resume / stop / reset) and nudges `WidgetCenter.shared.reloadTimelines`. The Widget Extension target is added and committed (2026-09-21) and the `.appex` embeds in `WatchApp.app/PlugIns/`. Checkbox stays `[ ]` for a different reason: **nothing has ever rendered on a watch face**. `simctl` cannot add a complication to a face and offers no touch or crown input, so this is *build-verified* only — see [decisions.md § 1713](../../docs/architecture/decisions.md)
...
- `PhoneSettingsTests.swift` — the three settings `PhoneSettings.swift` stores: each envelope reader's accepted values and its fail-closed refusals (a `stroller` default, a rogue privacy word, a ladder that is descending, out of 40–240, the wrong length or not integers), an empty ladder decoding as an instruction to clear, the picker priming rule and `WorkoutManager` honouring it, the zone boundaries, `stop()` stamping `isPublic` from the stored default (and nothing when there is none), the checkpoint round-trip, and a recovered run keeping the visibility it was recorded under.
...
Run from a Mac: `xcodebuild test -project apps/watch_ios/WatchApp.xcodeproj -scheme WatchApp -destination 'id=<sim-udid>'`. Target the simulator by **id**, not by name — see the localisation note above for why. **They compile and pass: 284 tests green on Xcode 26.4 / watchOS 26.4** — including on a watch simulator created for the purpose and confirmed to be in no pair, which is what settles whether the companion declaration keeps the app installable there ([decisions § 1679](../../docs/architecture/decisions.md)). Anything claiming this tier "has never been through a Swift compiler" is stale for this project. What the tests do NOT prove is anything about a wrist: no `HKWorkoutSession`, no `CLLocationManager`, no `WCSession` transfer has run on hardware. What remains outstanding is **device** verification, not compilation — one run syncing end to end from a paired physical watch and iPhone.
