...
<style>
	...
	/* --brand-ramp, the one declaration the landing hero paints too, so the
	   screen a visitor clicked through from and the one they sign in on
	   cannot drift apart. Every ink on it is measured at every stop, bare and
	   under each of this panel's veils, by gradient_foreground_guard.test.ts. */
	.auth-panel {
		position: relative;
		overflow: hidden;
		isolation: isolate;
