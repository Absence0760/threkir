...
- **Blocked on external credentials / approval** (code built and fail-closed in every case below — these are deploy steps, not unwritten features). Garmin Connect (developer-program application), native push FCM / APNs (Firebase + APNs keys + the per-app `google-services.json` / `GoogleService-Info.plist`; the client token registration itself shipped in `push_messaging_bridge.dart`), RunSignUp race results (API key), live in-app-purchase sheets (RevenueCat dashboard config).
...
- [ ] watchOS complication: pace + distance — Swift source + provider + four `widgetFamily` views (Circular / Corner / Inline / Rectangular) ship in `apps/watch_ios/Complications/`. `WorkoutManager` writes an active-run snapshot to an App-Group `UserDefaults` on every state transition (`publishComplicationSnapshot`) and nudges `WidgetCenter.reloadTimelines`. The `WatchAppComplication` Widget Extension target is added and committed (2026-09-21) in both `WatchApp.xcodeproj` and `Runner.xcodeproj`, so the source compiles on every PR and the `.appex` embeds in `WatchApp.app/PlugIns/`. **Checkbox stays unticked for a different reason: nothing has ever rendered on a watch face.** `simctl` cannot add a complication to a face and offers no touch or crown input, so this is *build-verified* only and needs a paired physical watch (#922) — see [decisions § 1713](../architecture/decisions.md).
...
- [x] **Phase 4b — email delivery + event-day reminders:** The *in-app* inbox fan-out for club posts + completed runs ships (migration `20261101_001`, persona #38 — `decisions.md § 96`); the **email leg of the device-delivery layer now ships too** (migration `20261130_001`, `decisions.md § 117`). A new `event_reminder` kind + `enqueue_event_reminders()` (hourly pg_cron) creates a reminder for every `going` RSVP whose occurrence is inside the next 24 h (skips cancelled occurrences; deduped per occurrence). An AFTER-INSERT trigger on `notifications` enqueues a `notification_email` job per recipient; the Go worker's `handler_notification_email.go` drains it, honours the `user_settings.prefs.email_notifications` preference (`all | important | off`, default `important` — `docs/backend/settings.md`), resolves the address via the GoTrue admin API, and sends over SMTP (local Mailpit `:24325`; Resend/SES SMTP in prod). Credential-free and end-to-end testable against the local Docker stack — email needed no Firebase/APNs key. Admin-update fan-out rides the same `notification_email` path once an admin-update notification kind is added.
- [~] **Phase 4b — native push (FCM / APNs):** Code complete; the open half is the credential step, tracked as **Native push delivery** in [`followups.md`](followups.md). The remaining device-delivery leg — native push to iOS + Android, both over FCM — is now **built, gated on credentials** (migration `20270212_001`, `native_push` kind). A third consumer of the `notifications` rows copies the shipped `web_push` sibling: the FCM HTTP v1 sender (`apps/job_worker/internal/nativepush/`) + handler (`handler_native_push.go`, reuses the `push_notifications` pref gate), the device-token-gated enqueue trigger + `clear_device_token` prune RPC, and the mobile `firebase_messaging` token registration (`push_messaging_bridge.dart`, both twins — register on sign-in, re-register on refresh, remove on sign-out, per-device `is_notifications_enabled` filter). `device_tokens` (migration `20260506_001`) finally has its client write path. Going live is blocked only on user-supplied Firebase credentials on the worker (`FCM_*`) + the per-app config files (`google-services.json` / `GoogleService-Info.plist`), plus the APNs auth key uploaded into the Firebase project for Apple delivery; unset → jobs finish done, rows stay pending, the mobile bridge no-ops. See `docs/features/native_push.md` + `decisions.md § 166` + `§ 1682` (why iOS goes through FCM rather than direct APNs). (Web push is a separate slice — the VAPID path already ships; see `parity.md`.)
...
- [ ] **Email — production ops:** ~~provision SMTP (Resend/SES) + `APP_BASE_URL` on the worker~~ (done, #922), confirm the pg_cron schedules in prod Supabase, ~~set SPF/DKIM/DMARC on `threkir.com`~~ (done, #922). Nothing engagement-side sends in prod until the pg_cron check and the send sign-off land — see [docs/features/email.md § Production ops](../features/email.md#production-ops-required-before-any-email-actually-sends).
