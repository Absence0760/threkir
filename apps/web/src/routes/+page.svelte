<script lang="ts">
	...
	import { DEMO_DISTANCE_LABEL, DEMO_HR_ZONES, DEMO_SPLITS, DEMO_TRACK } from '$lib/marketing/demo_preview';
		...
		<article class="feature" use:reveal>
			...
			<div class="feature-visual feature-visual--map" use:spotlight>
				...
				<div class="map-chip" aria-hidden="true" data-pop>
					...
					<span class="map-chip-value">{DEMO_DISTANCE_LABEL}<small>km</small></span>
				</div>
			</div>
		</article>
...
<style>
	...
	/* --brand-ramp is the wordmark's gradient taken to a legible depth; it
	   ...
	   product. The sign-in shell's panel paints the same token, so the first
	   screen a visitor sees and the one they sign in on cannot drift apart.
	   Every stop and every veil below is measured in
