...
- [x] GPS self-heal retry — `WorkoutManager.selfHealGps` on a 10 s poll, deciding through the pure `GpsHealth.retryTrigger`; plus `didFailWithError` (stops updates on everything but the transient `.locationUnknown`) and `locationManagerDidChangeAuthorization` (restarts them on a grant that lands mid-run). Wear's dead-subscription trigger is deliberately not ported — `CLLocationManager` has no subscription that can die. **Host-tested only**: a simulator cannot wedge CoreLocation, so no real dropout has been observed ([decisions § 1656](../architecture/decisions.md)).
- [x] Indoor / no-GPS mode (time-only) — the in-run banner reads "No GPS — time only" until a fix lands and "GPS lost" after one does, from `GpsHealth.banner` over the age of the last ACCEPTED fix. The clock always ticked and `writeTrackJSON` already wrote a valid `[]` for a zero-point run, so a treadmill session syncs over `WCSession` unchanged.
