...
const DOC = (...entries) => entries.map((n) => `## ${n}. Entry ${n}\n\nProse.\n`).join('\n');
...
test('both endpoints of a §§ A-B range are checked', () => {
	...
	assert.ok(
		errors.some((e) => e.includes('`§ 12` names no section')),
		errors.join('\n'),
	);
});
...
test('an UNRESOLVED number is tolerated where it is cited', () => {
	const { errors } = withTree(
		...
		(d) => check(d, { 1810: 'a reason long enough to be a reason, recorded rather than guessed at' }),
	);
	assert.deepEqual(errors, []);
});
...
test('an UNRESOLVED entry whose number now exists fails as stale', () => {
	...
	assert.ok(
		errors.some((e) => e.includes('Section 1810 now exists')),
		errors.join('\n'),
	);
});
...
test('an UNRESOLVED entry nothing cites any more fails as stale', () => {
	...
	assert.ok(
		errors.some((e) => e.includes('UNRESOLVED lists 1810')),
		errors.join('\n'),
	);
});
...
test('a doc whose heading shape stopped parsing fails rather than passing', () => {
	...
	assert.ok(
		errors.some((e) => e.includes('Parsed no `## N.` heading')),
		errors.join('\n'),
	);
});

test('a tree with no § reference at all fails rather than passing', () => {
	const { errors } = withTree({ [ADR_DOC]: DOC(10) }, (d) => check(d, {}));
	assert.ok(
		errors.some((e) => e.includes('Parsed no `§ N` reference')),
		errors.join('\n'),
	);
});
