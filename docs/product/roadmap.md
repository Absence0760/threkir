...
- [~] **Phase 4b — native push (FCM / APNs):** Code complete; the open half is the credential step, tracked as **Native push delivery** in [`followups.md`](followups.md). The remaining device-delivery leg — native push to iOS + Android, both over FCM — is now **built, gated on credentials** (migration `20270212_001`, `native_push` kind). A third consumer of the `notifications` rows copies the shipped `web_push` sibling: the FCM HTTP v1 sender (`apps/job_worker/internal/nativepush/`) + handler (`handler_native_push.go`, reuses the `push_notifications` pref gate), the device-token-gated enqueue trigger + `clear_device_token` prune RPC, and the mobile `firebase_messaging` token registration (`push_messaging_bridge.dart`, both twins — register on sign-in, re-register on refresh, remove on sign-out, per-device `is_notifications_enabled` filter). `device_tokens` (migration `20260506_001`) finally has its client write path. Going live is blocked only on user-supplied Firebase credentials on the worker (`FCM_*`) + the per-app config files (`google-services.json` / `GoogleService-Info.plist`), plus the APNs auth key uploaded into the Firebase project for Apple delivery; unset → jobs finish done, rows stay pending, the mobile bridge no-ops. See `docs/features/native_push.md` + `decisions.md § 166` + `§ 1677` (why iOS goes through FCM rather than direct APNs). (Web push is a separate slice — the VAPID path already ships; see `parity.md`.)
