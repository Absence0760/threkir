...
/// unless they choose "Keep public" in the post-stop dialog.
///
/// Harness mirrors run_screen_recording_flow_test.dart (geolocator /
...
void main() {
  ...
  group('RunScreen — post-live visibility (issue #664)', () {
    testWidgets(
        'stop with an active broadcast prompts instead of silently flipping public',
        (tester) async {
      ...
      expect(s.api.calls, isNot(contains('makeRunPublic')),
          reason: 'the stop path must never flip the run public unprompted');
      expect(s.api.calls, contains('conclude'),
          reason: 'the broadcast is still concluded for the spectator page');
      ...
      expect(s.api.calls, contains('makeRunPrivate'),
          reason: 'declining must actively clear the live stub\'s '
              'is_public=true (covers a failed cloud save too)');
      expect(s.api.calls, isNot(contains('makeRunPublic')));
      expect(s.runStore.captured, hasLength(1),
          reason: 'the visibility step is auxiliary — the run is saved');

      await drainAndUnmount(tester);
    });

    testWidgets('"Keep public" is the explicit act that keeps the run public',
        (tester) async {
      final s = await pumpLiveRunScreen(tester);
      await shareAndRecord(tester, s.api);
      final stopped = await holdFinish(tester,
      ...
      expect(s.api.calls, isNot(contains('makeRunPrivate')));
      expect(s.runStore.captured, hasLength(1));
    ...
    testWidgets(
        'a public privacy default needs no dialog — the save already honoured it',
        (tester) async {
      final s = await pumpLiveRunScreen(tester, privacyDefault: 'public');
      await shareAndRecord(tester, s.api);
      ...
      expect(find.byKey(const ValueKey('live-share-keep-public-dialog')),
          findsNothing,
          reason: 'their own default is public — nothing to resolve');
      expect(s.api.calls, contains('saveRun(isPublic: true)'),
          reason: 'the save carries the public default');
      expect(s.api.calls, isNot(contains('makeRunPublic')));
      expect(s.api.calls, isNot(contains('makeRunPrivate')));
