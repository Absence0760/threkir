...
Two things are deliberately outside all of this. A test of a bare global — `typeof window === 'undefined'`, `typeof localStorage` — asks whether there is a browser at all, which every module that also runs on a share Lambda or under prerender has to ask; that is not a question about a browser's age. And `apps/web/lambda` is Node 24 with full ICU, so the floor does not apply to it at all — which is exactly how the crawler-facing `<head>` and the tab's can differ. See [decisions.md § 1658](decisions.md).
