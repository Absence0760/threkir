...
### `apps/mobile_android/test/settings_sync_test.dart` — 41 tests
...
**Apple Watch zone mirror (3 tests, #950):** sign-in derives the wrist's `hr_zone_cutoffs` ladder from the bag (a `max_hr_bpm` of 200 → `[120, 140, 160, 180, 200]`) and pushes it; an edit through `updateUniversal` reaches the wrist without a relaunch, with no signal an explicit EMPTY ladder rather than the legacy 190 one, and a cleared `hr_zones` clearing it again; and a service that cannot answer `effective` does not fail the bag write it follows (L4).
...
### `apps/mobile_ios/test/` — 581 files, byte-for-byte
...
### `apps/web/src/lib/settings/weekly_goal.test.ts` — 9 tests

The web half of the `weekly_goal` parity pair (issue #902 § 5): the weekly distance goal typed in km or mi, stored as `weekly_mileage_goal_m` metres. Pins the registered key, the one-decimal display in either unit, that an absent / non-numeric / non-positive stored value shows nothing, that a sub-precision stored goal shows as 0 and is refused, whole-metre storage (50 km → 50000, 31 mi → 49890, 26.2 mi → 42165), a sweep of every one-decimal goal in 0.1–500 through a save and a reload in both units, that re-saving the figure the field shows hands back the stored metres unchanged, that an empty / unparseable / non-positive entry clears, and the usable range. Mirror suite below; the two keep the same nine cases.

### `apps/mobile_android/test/weekly_goal_test.dart` — 9 tests

The Dart half of the `weekly_goal` pair, case for case with `weekly_goal.test.ts` (web's `'km' | 'mi'` is `DistanceUnit`).

### `apps/mobile_android/test/settings_preferences_weekly_goal_test.dart` — 3 tests

The mobile weekly-goal editor on Settings → Preferences: a miles runner's 50000 m goal pre-fills the dialog as 31.1 with an "mi" suffix, saving that untouched figure pushes 50000 back unchanged, and 42.2 typed in kilometres pushes 42200.

### `apps/web/src/lib/settings/prefs_save_queue.test.ts` — 5 tests

The pure coalescing auto-save queue every preference page shares (decisions § 1640), on injected timers: edits inside the debounce go out as one write, a later edit to a key replaces the earlier one, the status reads saving → saved → idle, flushing with nothing pending writes nothing, and a failed batch is reported and merged back under an edit made while it was in flight.
...
### Mobile nutrition tests (Dart) — `apps/mobile_android/test/food_search_test.dart` (43) · `nutrition_totals_test.dart` (4) · `nutrition_targets_test.dart` (19, mirror) · `nutrition_screen_test.dart` (28 widget) · `nutrition_log_sheet_test.dart` (11 widget)
...
### `apps/watch_wear/android/app/src/test/kotlin/**/*Test.kt` — 834 Wear OS Kotlin/JUnit tests across 81 files
...
### `scripts/check_xcstrings_parity.test.mjs` — 19 tests
...
Seven cases added 2026-09-18 for the second catalog, `WatchApp/InfoPlist.xcstrings`, which localizes the four `NS*UsageDescription` consent prompts ([decisions § 1675](../architecture/decisions.md)). The guard now derives the shipped locale set across BOTH catalogs, so one running ahead of the other is a failure rather than each being graded against its own smaller set — a case pins that by adding a locale to the plist catalog alone and asserting the UI catalog's entries fail. Two of the new cases pin rules the second catalog needs and the first does not: an entry there may not leave the source language implicit, because its key is a plist key name rather than the English text, and its source value must equal the `Info.plist` string watchOS falls back to. Two more hold the key sets against each other in both directions — a usage description added to the plist with no catalog entry (the shape that shipped these four in English for the app's whole life) and a catalog entry naming a key the plist does not declare. The last is vacuity: an empty plist catalog must fail loudly rather than leave the claim unmade. An eighteenth case derives the sentinel key the absence cases mutate with instead of restating it, so a key that stops being absent from the tree fails here rather than silently turning those cases into positive controls.

One case added 2026-09-23 when the claims moved into `scripts/xcstrings_parity.py`, the engine the watch and the iPhone now share (#964): each catalog must be a member of the `WatchApp` target's Resources phase, since a catalog on disk the target never copies passes every other claim while the wrist reads the `Info.plist` English. The suite now stages a repo-shaped tree (`apps/watch_ios/` plus `scripts/xcstrings_parity.py`), because the wrapper execs the engine by its path relative to the app.

### `scripts/check_mobile_ios_xcstrings_parity.test.mjs` — 13 tests

The iPhone's consent-prompt guard (`apps/mobile_ios/scripts/check_xcstrings_parity.sh`, the same engine as the watch's) holds `ios/Runner/InfoPlist.xcstrings` to the eleven `NS*UsageDescription` keys in `Info.plist`. Until #964 that catalog did not exist, so every permission prompt the phone raises — HealthKit read and write, both location levels, camera, Bluetooth, motion, photos, calendar — was English in all seven locales, and nothing could see it: the phone's UI strings are ARB and fully translated, and iOS reads these few out of the bundle instead. Measured the way the watch suite is, by mutating a repo-shaped copy of the real tree with the unmutated copy as the positive control. The cases are the ones that depend on the phone's own wiring: a permission added to the plist with no entry and an entry whose key the plist dropped; a locale an entry never translated; an entry with no English, and a catalog with no `sourceLanguage` at all; English drifted from the plist fallback; a plist value carrying an XML entity, which must compare decoded rather than fail a correct pair; `pt-PT` dropped from `CFBundleLocalizations` and from `knownRegions`; the catalog dropped from the `Runner` target's Resources phase; and the Runner build file repointed at the WATCH app's `InfoPlist.xcstrings`, which `Runner.xcodeproj` also references because it builds the watch too — the one mistake a filename match would pass. Two more are the positive control and the absent-key sentinel. The engine's generic claims (plurals, the `ja` exemption, empty catalogs) are exercised by the watch suite, which has the UI catalog to exercise them on. Run as a step of the `ios-native-declarations` job with its own `::error::`.

### `scripts/check_watch_ios_source.test.mjs` — 155 tests

`apps/watch_ios` is compiled by one job, on a macOS runner, and every claim about it otherwise rests on reading. `scripts/check_watch_ios_source.mjs` makes the claims a bare `node` on Linux can honestly make — the count lives in its own output rather than in a sentence here, which is how this one came to say six while there were fourteen — and every failure it exists to catch is silent on the platform: a `Text("…")` with no String Catalog entry falls back to the key, so the watch renders English in every locale and nothing throws; an entitlement nothing exercises builds, links and ships, and is refused months later by App Review; two copies of one formatter drifting apart leaves the Swift suite green because `ActiveRunComplication.swift` is in no target and `@testable import WatchApp` therefore links only the other copy; and a metadata key one end of the WCSession run hand-off sends and the other never lifts out costs the row a column with nothing raised anywhere. So the guard is measured the way its sibling above is — a copy of the real tree mutated into each shape it must refuse, with the unmutated copy as the positive control. Sixteen cases are those mutations, one per refusal in each direction: a literal with no key, an accessibility hint with no key, a catalog entry nothing references, a purpose string dropped while its call stays and one declared with no call, a background mode each way, the HealthKit entitlement dropped, an unclaimed entitlement (the live `healthkit.background-delivery` defect, replanted), an App Group entitlement present but empty, a formatter that drifted and one deleted outright, an App Group renamed in Swift but not in the README, and a metadata key dropped on each end. Three assert the guard does NOT fire where it must not — `Text(verbatim:)` needs no entry (or the fix the guard recommends would fail the guard recommending it), a literal inside a comment is not a key, and a key reached only through `String(localized:)` is not reported as dead. Two are vacuity: a tree with no Swift, and an empty catalog. The rest measure the parsers directly, each against the specific way it was first written wrong — comment stripping that ate `http://` out of a URL literal, an interpolation matcher that stopped at the inner paren of `\(Int(bpm.rounded()))` and reported a live call site as missing, a dictionary scan that ended at a nested `]` and lost `last_modified_at`. `parseFlatPlist` is additionally asserted against the two real files it reads, because a hand-rolled reader written to avoid a dependency is only worth having if it answers what the dependency would. Two more cover the claim that a shipped password grant keeps its session in the Keychain — the compensating control for allowing a grant to ship at all once the wrist grew its own sign-in: the Keychain item class renamed away, and a refresh token written to `UserDefaults` while the store stays. Neither is visible to Swift; both compile, run and sign in. Five more cover claim (15), the one that holds the Stop control to a HELD press: a `Button` wired straight to `stop()`, one of the two call sites losing its hold, the press duration no longer being what fires it, no stop call left to read at all, and a duration constant the guard cannot parse. Nothing in Swift can see any of them — a `Button` whose closure calls `stop()` is the same program as one wired to a hold. Run as a fourth step of the ungated `watch-ios-locale-parity` job with its own `::error::`. See [decisions § 884](../architecture/decisions.md) – [§ 888](../architecture/decisions.md).
...
Fifteen cases added 2026-09-18 for claim (15), the one that holds the two Xcode projects that now build this app against each other ([decisions § 1679](../architecture/decisions.md)). `Runner.xcodeproj` gained a `WatchApp` target referencing the same sources, so membership, the bundle-defining settings and the Embed Watch Content phase are each transcribed twice and drift in silence: a source in the watch project alone is exercised by `test-watch-ios` and absent from every `.ipa`, one in the phone project alone ships to a wrist compiled by nothing that runs a test, and deleting the copy phase un-ships the tier while both projects build and both suites pass. Nine mutations are those refusals — a source dropped on each side, a resource dropped, a bundle identifier changed, an `INFOPLIST_FILE` repointed at a second copy, the copy phase removed, its destination changed, the watch target renamed away, and the target dependency deleted — plus one vacuity case where the membership becomes unreadable, which claim (13) cannot see because it reads the `PBXBuildFile` rows rather than the phase's file list. The last five drive the new pbxproj parsers directly: `nativeTarget` reading the block's own `name` rather than the trailing comment Xcode does not rewrite on a rename, `targetPhaseMembers` refusing to let a sibling target's phase answer for the one under test, `targetConfigurations` walking the target's own configuration list, `settingValue`, and `pbxObject` stopping at its object's closing brace.

Three cases added 2026-09-20 for the seventeenth claim, which is the only one in the file not about Swift or a plist: no two Xcode objects may share an id, in either project. The ids in these two `project.pbxproj` files are hand-assigned rather than Xcode-generated, so two branches allocating the next free id from the same base collide — and land in non-adjacent parts of the file, which git merges cleanly. `InfoPlist.xcstrings` (#951) and `RunLaps.swift` (#954) reached `main` sharing one, and nothing failed: Xcode keeps one object and silently drops the other, so a listed file is simply not compiled. The three cases are the collision itself, that the message names BOTH objects (the id alone does not say what was dropped), and vacuity — a project file whose shape stopped parsing must fail rather than pass, because that is the state where a collision is most likely to be sitting unread.

Seven cases added 2026-09-20 for the eighteenth claim, which moved here from `apps/mobile_android/test/architecture_guards_test.dart` and its iOS twin ([decisions § 1687](../architecture/decisions.md), issue #965). The old group reached across apps by relative path to assert that six hint SENTENCES appeared verbatim in Swift it did not own, so renaming Stop's hint for hold-to-stop failed `Test Flutter packages` on 1 of 7,435 tests and took `CI gate` with it — on a copy edit. It also opened with `if (!file.existsSync()) return;`, so a moved directory would have made it pass silently forever. The claim here asserts the PROPERTY instead: a run control has a hint. Two cases pin that directly — one drops a hint and fails, one rewords a hint and stays silent — and the rest hold the two structural exemptions (a `confirmationDialog` action, whose dialog carries the cue, and a `#if DEBUG` control that ships to nobody) plus the staleness of the two-entry `HINTLESS_CONTROLS` register.

One case added 2026-09-23 for claim (20)'s watch rail, which now reads the whole `PhonePreferences` type rather than two named methods (#950): renaming `hr_zone_cutoffs` in one of the three readers added beside the originals is refused on both sides, so a reader nobody lists is on the rail the moment it is written. The shipped-tree case now expects five settings-push keys, and the vacuity mutation renames every `payload[` subscript in the type.
...
### `scripts/check_release_table.test.mjs` — 13 tests

`docs/ops/releasing.md`'s release table is what an operator reads before cutting a tag, and nothing compared it with the workflows: the `mobile_ios@*` row promised the Release carries an `.ipa` that the workflow's own `--no-codesign` build cannot produce, and the attach step globbed a path matching nothing while `softprops/action-gh-release` reported success ([decisions § 1673](../architecture/decisions.md)). The guard resolves each row through the doc's own tag-to-workflow list and re-derives the two mechanical columns — the runner from `runs-on:`, the artifact claim from whether a LIVE upload step exists and what its `files:` glob names — plus the `fail_on_unmatched_files: true` rule that makes the second answer mean anything. The suite measures both directions of each refusal: a row promising an artifact no live step produces AND a row understating a release that does attach one; a table row no workflow is mapped for AND a mapped workflow with no row; an extension disagreeing with the glob, a mis-stated runner, and a missing `fail_on_unmatched_files`. A commented-out upload step must read as absent, which is asserted on its own, because `release-ios.yml` carries exactly that block awaiting signing and counting it live would pass the original bug. A parse matching nothing must be a hard error rather than a pass, and the last two cases run the parsers and the whole check over the SHIPPED doc, so the suite fails if the table drifts even when every synthetic case passes. Runs as a step of the existing `Doc registry drift` job with its own `::error::`, so it needs no new `CI gate` `needs:` entry ([§ 1674](../architecture/decisions.md)).
...
### `apps/backend/supabase/tests/gym_repeated_exercise_key_test.sql` — 9 tests
...
### `apps/backend/supabase/functions/**/*.test.ts` — 970 deno tests across 72 files
...
**Route mocks that must fire (`fixtures/mock-route.ts`)** — a `page.route` pattern that stops matching is silent: the request reaches the real endpoint, the stub's body is never used, and the assertions written beside it are scored against a handler nothing invoked. `mockRoute` counts its handler's invocations and fails the case from fixture teardown when the count is zero, naming the pattern and the registering line; `{ neverFires: 'why' }` inverts it to a zero-assertion for the mocks that are meant to stay silent and fails when one starts firing, and `mockRoute.neverFires(pattern, 'why')` says the same from inside a case about a `beforeEach` stub it does not exercise — a declaration that names a pattern no mock in that test registered fails, so neither sentence can go stale. 95 call sites across 46 specs are registered through it — every mock that counts its own hits, and every success stub in front of a GoTrue / Edge Function / `/api/` endpoint. `fixtures/mock-route.test.ts` (7 tests, web unit suite) derives that rule from the call sites themselves and fails the PR on a new bare `.route()` that meets it, and `cross-cutting/mock-route-fires.spec.ts` (6 tests) proves the instrument by letting three deliberately-wrong mocks fail under `test.fail()`. Switching it on found fourteen dead mocks, six of them share-page cases finishing in 140–450 ms against a half-mounted page ([decisions § 1667](../architecture/decisions.md)).
...
```
tests-e2e/
  fixtures/                    — globalSetup, helpers, browser-zone dates, route-mock liveness, seeded constants, users
  ...
  dashboard/tap-targets.spec.ts — every icon button on /dashboard measured against the 44 px bar at 1280x720 and 375x667, through fixtures/tap-targets.ts (which comment-tap-targets.spec.ts reads too). Fails a control that misses its floor OR meets it exactly — the headroom half, see decisions § 1666 — and fails a sub-bar control that is not in the inline-disclosure registry
  feed.spec.ts                 — /feed
  ...
  plans/
    ...
    reading-order.spec.ts      — /plans/[id] leads with today's session then this week, and every secondary block (Plan progress, Calendar, Week by week, Share & publish) follows them as a named `<details>` (decisions § 1660); second case shuts the Calendar expander, waits for the per-account blob `util/disclosure_state.ts` writes, reloads and finds it still shut with its siblings untouched. Asserted on the `<details>`'s own `open`, because Chromium hides a closed one through `::details-content`, which Playwright still reports as visible
  ...
  settings/
    ...
    preferences.spec.ts        — the preference pages the old /settings/preferences was split into (issue #905): theme, units, pace format, map style on /settings/display, the email + push channels and optional emails on /settings/notifications, resting + max HR on /settings/training, demographics on /settings/body (each save round-trip); the landing page listing every group; the #heart-rate-zones / #weekly-mileage-goal / #body-metrics redirects; a control's one-line explanation as its accessible description
    preferences-load-failure.spec.ts — /settings/body with get_my_profile failing: the shared PrefsPage shell shows an alert with Retry instead of the form (no Save, no auto-save levers), and Retry restores it
    weekly-goal.spec.ts        — the weekly distance goal typed in km or mi on /settings/training lands in the bag as metres (42.2 km → 42200, 26.2 mi → 42165), reads back in the chosen unit, an untouched blur writes back the stored metres rather than the rounded display, and 600 is refused in the field without riding out on the next coalesced write (issue #902 § 5)
    layout.spec.ts             — the settings nav's four sections (Profile / Preferences / Apps & data / Account & legal), every tab present and routing, and no tab for the /settings/preferences landing page
    privacy-zones.spec.ts      — /settings/privacy privacy-zones CRUD: service-role plant a zone via user_settings.prefs.privacy_zones, reload, verify the zone-row renders with seeded coords + radius, click Remove + Save → reload → zone gone from UI AND prefs blob; empty-state copy when prefs has no zones. The MapLibre tap-to-add picker isn't drivable in Playwright, so the seed path is via service-role; the share-time guardrail is covered separately in cross-cutting/privacy-zones.spec.ts
...
stack port in the PEER column is not a holder, and `124322` is not `24322`.
The five #963 cases pin the durable half: every host port `config.toml`
publishes is pinned (an unpinned one falls back to the CLI's 5432x default,
inside the ephemeral range) and below 32768, the script's `PORTS` is exactly
that set, and `RESERVED_RANGE` reserves exactly `PORTS`. A synthetic config
with db on 54322 and studio unpinned is asserted to produce both violations,
