...
- **Tap `Log` = the primary capture action, derived from data presence**
  (decisions § 1642). Until the user has logged a lift or a meal, the tap
...
- **System back walks toward Home** and only leaves the app from Home; a
  live recording raises a confirm first (decisions § 1642).
...
not enough** (decisions § 1642): a runner who never opened Settings still paid
...
| Runner protection | One-tap run start **derived from data presence** — on until a lift or a meal is logged (`runIsPrimaryLogAction` in `preferences.dart`, decisions § 1642); `Preferences.keepRunPrimary` + the `settings_preferences_screen.dart` switch remain as the explicit override. Long-press always opens the fan |
