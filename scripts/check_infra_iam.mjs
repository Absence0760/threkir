...
//      deploy role. Since decisions § 1656 the credentials are encrypted at
...
//      and for the opposite reason. It was unexercised until § 1656 and is
  ...
  if (ciphertexts.length === 0) {
    errors.push(
      'no `aws_kms_ciphertext` resource in the web-stack module. The coach and generate-route ' +
        'credentials reach their functions as one encrypted blob each (decisions § 1656); with ' +
    ...
    for (const key of keys) {
      ...
      errors.push(
        `local.${local} puts the sops key ${key} in a function environment as PLAINTEXT. Every ` +
          ...
          '(decisions § 1656). Encrypt it into that function\'s `aws_kms_ciphertext` blob, or, if ' +
...
export function checkDecryptGrant(
  ...
  // cycle). Unexercised until § 1656 and load-bearing since: it is what decrypts
  ...
  if (ciphertextLabels.length > 0 && execRole === undefined)
    errors.push(
      "the module's kms:Decrypt statement no longer names the Lambda execution role " +
        ...
        'carrying Lambdas answer 503 on every cold start (decisions § 1656).',
    );
